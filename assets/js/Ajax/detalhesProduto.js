$(function(){
    ...
    $(".addCarrinho").click(function(e){
        ...
        if(token == null)
            ...
            $.ajax({
                url: 'https://localhost:44392/api/Carrinho/' + id,
                type: "GET",
                dataType: "json",
                contentType: "application/json; charset=utf-8",
                ...
                success: function (result) {
                    if (result == null)
                        adicionarCarrinho(result);
                    else
                        atualizarCarrinho(result);
                },
                error: function (err) {
                    console.log(err);
                },
    ...
    function adicionarCarrinho(result){
    ...
    function atualizarCarrinho(result){
        carrinho = {
            "id": parseInt(result.id),
            "idProduto": parseInt(id),
            "qtde": parseInt($(".counter-btn").val()) + parseInt(result.qtde),
            "preco": parseFloat(precoUnitario) * (parseInt($(".counter-btn").val()) + parseInt(result.qtde))
        };
